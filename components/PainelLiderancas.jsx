...
export default function PainelLiderancas() {
  ...
  useEffect(() => {
    ...
    liderancas.forEach((l) => {
      ...
      const icon = L.divIcon({
        ...
        html: `<svg class="leader-pin-svg" width="32" height="38" viewBox="0 0 32 38" xmlns="http://www.w3.org/2000/svg" aria-label="${escapeHTML(cat.corNome)}"><path d="M16 36C16 36 29 23.8 29 14.4C29 6.95 23.18 2 16 2C8.82 2 3 6.95 3 14.4C3 23.8 16 36 16 36Z" fill="${cat.cor}" stroke="${stroke}" stroke-width="3"/><circle cx="16" cy="14" r="4.5" fill="rgba(255,255,255,.82)" stroke="rgba(0,0,0,.22)" stroke-width="1"/></svg>`,
        iconSize: [32, 38], iconAnchor: [16, 38], popupAnchor: [0, -34]
      });
      const meta = metaLideranca(l);
      const pct = meta > 0 ? Math.min(100, Math.round(((Number(l.atuais) || 0) / meta) * 100)) : 0;
      const html = `<div class="pin-popup"><h4>${escapeHTML(l.nome)}</h4><div>${escapeHTML(l.local)} · ${escapeHTML(info.nome)}</div><div class="pin-cat"><span style="background:${cat.cor};border-color:${corPinBorda(cat)}"></span>${escapeHTML(cat.corNome)} — ${escapeHTML(cat.nomeCurto)}</div><p>Meta: <b>${meta.toLocaleString('pt-BR')}</b><br/>Captados: <b>${Number(l.atuais || 0).toLocaleString('pt-BR')} (${pct}%)</b></p></div>`;
  ...
  const resumoRegionais = listaRegionais(mapaAtivo).map((r) => {
    ...
    return { ...r, qtd: itens.length, votos: itens.reduce((s, l) => s + (Number(l.atuais) || 0), 0), meta: itens.reduce((s, l) => s + metaLideranca(l), 0) };
  });
        ...
        <aside className="sidebar">
          ...
          <div className="lideranca-list">
            ...
            {itensFiltrados.map((l) => {
              ...
              const cat = categoriaInfo(l.categoria);
              return (
                <button key={l.id} className="l-card" style={{ borderLeftColor: cat.cor }} onClick={() => setEditando(l)}>
