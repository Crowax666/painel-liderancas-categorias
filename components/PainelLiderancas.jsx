...
export default function PainelLiderancas() {
  ...
  useEffect(() => {
    ...
    liderancas.forEach((l) => {
      ...
      const icon = L.divIcon({
        className: '',
        html: `<div style="width:16px;height:16px;border-radius:50% 50% 50% 0;background:${info.cor};transform:rotate(-45deg);border:2px solid rgba(20,27,35,.75);box-shadow:0 2px 5px rgba(0,0,0,.4)"></div>`,
        iconSize: [16, 16], iconAnchor: [8, 16], popupAnchor: [0, -16]
      });
      const meta = calcMVL(l.fel);
      const pct = meta > 0 ? Math.min(100, Math.round(((Number(l.atuais) || 0) / meta) * 100)) : 0;
      const html = `<div class="pin-popup"><h4>${escapeHTML(l.nome)}</h4><div>${escapeHTML(l.local)} · ${escapeHTML(info.nome)}</div><p>Meta: <b>${meta.toLocaleString('pt-BR')}</b><br/>Captados: <b>${Number(l.atuais || 0).toLocaleString('pt-BR')} (${pct}%)</b></p></div>`;
  ...
  async function exportarExcel() {
    try {
      ...
      ajustarColunas(wsTodas, [28, 24, 30, 14, 12, 12, 8, 12, 14, 10, 18, 18, 22, 36, 20, 20]);
      ...
      ajustarColunas(wsRanking, [28, 24, 30, 14, 12, 12, 8, 12, 14, 10, 18, 18, 22, 36, 20, 20]);
      ...
      ajustarColunas(wsPendencias, [28, 24, 30, 14, 12, 12, 8, 12, 14, 10, 18, 18, 22, 36, 20, 20]);
      ...
      ajustarColunas(wsContatos, [28, 18, 24, 30, 24]);
  ...
  function exportarCSV() {
    const linhas = [['nome', 'local', 'regional', 'mapa', 'lat', 'lng', 'fel', 'meta_mvl', 'votos_atuais', 'status', 'whatsapp', 'responsavel', 'observacao', 'criado_em', 'atualizado_em']];
    liderancas.forEach((l) => {
      linhas.push([
        l.nome, l.local, regionalInfo(l.mapa, l.regional)?.nome || '', l.mapa, l.lat, l.lng, l.fel, calcMVL(l.fel), l.atuais || 0,
  ...
  const dadosFormulario = editando || {
    ...
    fel: 1, atuais: 0, whatsapp: '', responsavel: '', observacao: ''
  };
      ...
      <section className="body-grid">
        ...
        <aside className="sidebar">
          ...
          <div className="lideranca-list">
            ...
            {itensFiltrados.map((l) => { const meta = calcMVL(l.fel); const pct = meta > 0 ? Math.min(100, Math.round(((Number(l.atuais) || 0) / meta) * 100)) : 0; return <button key={l.id} className="l-card" onClick={() => setEditando(l)}><div><b>{l.nome}</b><span>{meta.toLocaleString('pt-BR')}</span></div><small>{l.local} · {regionalInfo(l.mapa, l.regional)?.nome || ''}</small><i><em style={{ width: `${pct}%` }} /></i></button>; })}
          </div>
